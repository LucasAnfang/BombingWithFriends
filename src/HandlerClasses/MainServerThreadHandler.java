...
public class MainServerThreadHandler extends Thread {
	...
	private void generateHighScoreResponse( ObjectOutputStream objectOut) {
		HighScoreResponse response = new HighScoreResponse();
		Queue<ScorePair> highScores = null;
		highScores = mainServer.getHighScoreSheet();
		response.setHighScores(highScores);
		...
		try {
			...
			while(!moribund){
				...
				if(newMessage!=null){
					...
					else if(newMessage instanceof HighScoreRequest){
						generateHighScoreResponse(objectOut);
					}
				}
				/*
				try {
