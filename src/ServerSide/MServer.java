...
public class MServer implements Runnable{
	...
	public synchronized Queue<ScorePair> getHighScoreSheet(){
		return driver.returnFullScoreList();
	}
...
//			System.out.println(generatedPort + " " + playerCapacity);
			...
			System.out.println("Something is messed up on Shanu's end");
		}
	}
	public boolean processRegistration(String Username, String Password){
		if(!driver.doesExist(Username)){
			driver.registerUser(Username, Password,0);
	...
	public void run(){
		...
		while (!moribund){
			...
			try {
				...
				System.out.println("BEFORE CHECKING NOT NULLS");
				if(currentGameServers != null && !currentGameServers.isEmpty())
				{
					System.out.println(" CURRGAMESERVER WANT NULL");
					for(GameServer gs : currentGameServers)
					{
						if(gs!= null && gs.sl!= null && gs.sl.h!=null && gs.sl.h.gameover !=null)
						{
							System.out.println(" GS.SL.H WASNT NULL");
							if(gs.sl.h.gameover)
							{
								if(!gs.sl.h.winner.isEmpty() && !gs.sl.h.winner.equals(""))
								{
									System.out.println("UPDATING USER TOTAL SCORE");
									updateUserTotalScore(gs.sl.h.winner);
								}
							}
						}
					}
