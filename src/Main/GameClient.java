...
//			System.out.println("GOT HERE BEFORE WHILE WAITING FOR PLAYERLIST");
			...
			for(Player p : playerList)
			{
				...
				if(p.playerName.equals(this.username))
				{
					this.playerNo = p.getPlayerNo();
				}
				if(p.getPlayerNo() == 1)
				{
					System.out.println("SET SPAWN POINT FOR PLAYER 1");
					p.setX(16);
					p.setY(16);
				}
				if(p.getPlayerNo() == 2)
			...
			gbgui = new GameBoardGUI(playerList, username, servername, this, playerNo);
			c = gbgui.core;
...
//						}
				...
				if(obj instanceof ArrayList<?>)
				{
					...
					for(Player p : playerList)
					{
						if(p.playerName == username) //CHECK FOR USERNAMES HERE AND ASSIGN NUMBER
						{
							this.p = p;
							this.p.setPlayerNo(p.getPlayerNo());
						}
					}
...
//				}
	...
	public void createPlayer()
	{
		...
		p = new Player(16,16, playerNo, spriteSheetName, username);
	}
